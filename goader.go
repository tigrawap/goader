...
func configure() {
	...
	flag.StringVar(&config.urlsSourceFile, "url-source", NotSetString, "File to read urls from, instead of --url template(built manually or generated by --written-urls-dump)")
	...
	flag.StringVar(&config.writtenUrlsDump, "written-urls-dump", NotSetString, "Path to dump written files, so then can rerun as reads job")
